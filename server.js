...
function nightActors(room) {
  return activePlayers(room).filter((player) => ["mafia", "doctor", "detective", "medium"].includes(player.role));
}
...
function maybeAutoAdvance(room) {
  if (!room.settings.autoAdvance || room.phase === "ended") return false;
...
function applyNight(room) {
  ...
  if (!mafiaTargetId) {
    addLog(room, "밤에 마피아의 선택이 없어 아무도 습격당하지 않았습니다.");
...
function applyDay(room) {
  ...
  if (!targetId) {
    addLog(room, "낮 투표가 동률이거나 비어 있어 아무도 처형되지 않았습니다.");
  } else {
    const target = room.players.get(targetId);
...
function handleAction(id, message) {
  ...
  if (data.action === "settings") {
    ...
    if (!canTarget(player, target, room.phase) || room.phase === "ended") return;
    if (room.phase === "day") {
      ...
      if (player.role === "detective") addNotice(room, id, `${target.name} 님을 조사 대상으로 선택했습니다. 결과는 밤 종료 후 공개됩니다.`);
      if (player.role === "medium") addNotice(room, id, `${target.name} 님의 혼을 확인합니다. 힌트는 밤 종료 후 모두에게 공개됩니다.`);
